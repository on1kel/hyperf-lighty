...
if (! function_exists('get_controller_validation_request_classes')) {
    ...
    function get_controller_validation_request_classes(string|object $controller_class): array
    {
        if (is_object($controller_class)) {
            if (! ($controller_class instanceof On1kel\HyperfLighty\Http\Controllers\Controller)) {
...
if (! function_exists('generate_doc_request_by_request_class')) {
    ...
    {
        if (! ($request_class instanceof On1kel\HyperfLighty\Http\Requests\BaseRequest)) {
            $request = new $request_class();
        }

        if (! isset($request) || ! ($request instanceof On1kel\HyperfLighty\Http\Requests\BaseRequest)) {
            throw new RuntimeException('[DocRequestGenerator] Переданный класс не унаследован от On1kel\HyperfLighty\Http\Requests\BaseRequest');
        }

        /** @var On1kel\HyperfLighty\Http\Requests\BaseRequest $request */
...
if (! function_exists('app_path')) {
    function app_path(string $path = ''): string
    {
        ...
        return $path === '' ? $full : $full . '/' . ltrim($path, '/\\');
    }
}
